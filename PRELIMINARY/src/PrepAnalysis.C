...
void PrepAnalysis::Loop(const TString& which_analysis)
{
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if((jentry +1 ) % entry_CP == 0) std::cout<< "--> Processing event number " << jentry + 1 << std::endl;
	 if(!(bool)HLT_Dimuon25_Jpsi)continue;
  ...
  if (code == 0){
      TFile gen_outfile("./plots/analysis_Gen_UL17_HLTdiMu25.root","RECREATE"); // create the output file
      h_Gpt_mu.Write();
		h_GptL_mu.Write();
		h_GptSL_mu.Write();
      h_Geta_mu.Write();
      ...
      gen_outfile.Close();
		std::cout << " ----> Output written on file " << gen_outfile.GetName() << std::endl; 
      ...
      TFile mul_outfile("./plots/analysis_Multiplicity_UL17.root","RECREATE"); // create the output file
      h_nB0.Write();
      h_nK0s.Write();
      h_nJPsi.Write();
      h_nPiPi.Write();
      ...
      mul_outfile.Close();
		std::cout << " ----> Output written on file " << mul_outfile.GetName() << std::endl; 
  }  
...
void PrepAnalysis::gen_K0s_ptetaphi(const int &Mother_pdgId, ROOT::Math::PtEtaPhiMVector* V){
...
void PrepAnalysis::GenPart_FillKinHist(ROOT::Math::PtEtaPhiMVector* GenVec, TH1* h_pt, TH1* h_eta, TH1* h_mass){ 
...
void PrepAnalysis::GenPartFillP4(){
...
void PrepAnalysis::PrintDecayChain(){
