...
void MCbase_B0toX3872K0s::GenPartFillP4_X(){
   ...
   for (UInt_t g = 0; g < nGenPart; g++){
	...
	if(RhoIdx > 0){   
     	GenP4_PiPi.SetPt(GenPart_pt[RhoIdx]);   GenP4_PiPi.SetEta(GenPart_eta[RhoIdx]);
		GenP4_PiPi.SetPhi(GenPart_phi[RhoIdx]); GenP4_PiPi.SetM(GenPart_mass[RhoIdx]); 
	} else std::cout << "No generated rho is found" << std::endl;  
	
	if(X3872Idx > 0){   
      	GenP4_JpsiPiPi.SetPt(GenPart_pt[X3872Idx]);   GenP4_JpsiPiPi.SetEta(GenPart_eta[X3872Idx]);
		GenP4_JpsiPiPi.SetPhi(GenPart_phi[X3872Idx]); GenP4_JpsiPiPi.SetM(GenPart_mass[X3872Idx]); 
...
void MCbase_B0toX3872K0s::GenPartFillP4_Psi(){
   ...
   for (UInt_t g = 0; g < nGenPart; g++){
	...
	if(Psi2SIdx > 0){   
      GenP4_JpsiPiPi.SetPt(GenPart_pt[Psi2SIdx]);   GenP4_JpsiPiPi.SetEta(GenPart_eta[Psi2SIdx]);
		GenP4_JpsiPiPi.SetPhi(GenPart_phi[Psi2SIdx]); GenP4_JpsiPiPi.SetM(GenPart_mass[Psi2SIdx]); 
	} else std::cout << "No generated X(3872) is found" << std::endl;  
