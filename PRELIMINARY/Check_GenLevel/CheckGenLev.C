...
void CheckGenLev::Loop()
{
  ...
  TH1F h_nTracks("nTracks", "", 50, 100, 800);
   ...
   const ULong64_t percentToPrint = 10;
   float perc;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      perc = (jentry + 1)/nentries*100.;
      if ( (int)(perc) % percentToPrint == 0) std::cout << " ... " << perc << " \%"<< std::endl;
      // ------- GENERATOR -------//
      GenPartFillP4();
    ...
    TFile gen_outfile("./plots/CheckGenLev_DBGpreUL17.root","RECREATE"); // create the output file
    h_Gpt_mu.Write();
    h_GptL_mu.Write();
    h_GptSL_mu.Write();
    h_Gpt_pi.Write();
...
void CheckGenLev::GenPartFillP4(){
