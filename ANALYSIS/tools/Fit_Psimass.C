...
void Fit_Psimass(string dataset = "2017_Psi"){
...
void AddModel(RooWorkspace* ws, float low, float high){
    ...
    RooRealVar M_Psi("M_Psi2S", "X reco mass", low, high, "GeV");
    cout << " ... signal model for M(X)" << endl;

    // Double Gaussian 
    RooRealVar Xmass ("Xmass" , "", 3.68, 3.67, 3.69); 
    RooRealVar X_sigma1("X_sigma1", "", 0.005, 0.0001  , 0.01);
    RooRealVar X_sigma2("X_sigma2", "", 0.01, 0.005  , 0.05);
    
    RooGaussian G1signal("G1signal", "", M_Psi, Xmass, X_sigma1);
    RooGaussian G2signal("G2signal", "", M_Psi, Xmass, X_sigma2);
    // Double Side-CrystalBall
    RooRealVar X_sigma("X_sigma", "", 0.005, 0.0005  , 0.01);
    RooRealVar alphaL("alphaL", "", 1., 0.001, 10);
    RooRealVar nL("nL", "", 5, -10, 20);
    RooRealVar alphaR("alphaR", "", 1., 0.001, 10);
    RooRealVar nR("nR", "", 5, -10, 20);
    
    My_double_CB SGNmodel("SGNmodel", "", M_Psi, Xmass, X_sigma, alphaL, nL, alphaR, nR);
...
void AddData(RooWorkspace* ws, string dataset, float Mlow, float Mhigh){
    ...
    RooRealVar *M_Psi = ws->var("M_Psi2S");
    RooDataSet data("data", "data", *M_Psi, Import(*tree));
...
void FitToData(RooWorkspace* ws, string dataset, float Mlow, float Mhigh){
    ...
    // create the plot
    int Nbins = 50;
    RooPlot *frame = M_Psi->frame(Title("MC-matching reco X candidates " + TString(dataset)));
...
void SaveResults(RooWorkspace* ws, string dataset, float Mlow, float Mhigh){
	...
	// SIDEBANDS
    double norm       = model->createIntegral(*M_Psi, NormSet(*M_Psi))->getVal();
    ...
    int SR_Ns= 3;
    float SGNeff = data->sumEntries(Form("M_Psi2S>%f&&M_Psi2S<%f", Xm - SR_Ns*SIGMA, Xm + SR_Ns*SIGMA))/ data_N->getVal();
	...
	outFile << "### Psi2S signal region" << std::endl;	
